...
/**
 * Data structure to represent nodes of a search tree.
 */
class Node<S, A>{

    // the state of the node
    S state;
    // the sequence of actions to get from the start state to the current state
    List<A> actions;
    // the cost of this sequence of actions
    double pathCost;
    ...
    @Override
    public String toString() {
        return "(" + state + ", " + actions + ", "  + pathCost + ")";
    }
}



class Node2<S,A> extends Node implements Comparable{   // 创建一个comparable的 node结构.
    double f;

    public Node2(S state, List<A> actions, double pathCost, double f) {
        super(state, actions, pathCost);
        this.f = f;
    }
...
/**
 * A simple structure for storing information about solutions.
 */
class Solution<S, A> {
    public S goalState;
