        ...
        for (int i = 0 ; i < args.length ; ++i) {
            ...
            switch (s) {
                case "-n":
                    ...
                    //  ++1是自增后再 操作, 所以args[++i]是 "-n"参数 后面的, 第二个字符串参数    "NpuzzleMain  -n   xx,yy,zz"
                    //  split 返回拆分后的 字符串数组 [xx] [yy] [zz]
                    //  Arrays.stream 把 xx yy zz 转换成 stream 进行其他操作
                    //  mapToInt(Integer::parseInt) 将流里的元素 转换成 integer
                    //  最后还是转换回为 数组 toArray();
...
class NPuzzleHeuristic implements SearchHeuristic<NPuzzleSearchState,NPuzzleAction> {
    ...
    public Double value(NPuzzleSearchState state, SearchProblem<NPuzzleSearchState, NPuzzleAction> problem) {
        ...
        if (problem instanceof NPuzzleSearchProblem) {
            ...
            int[][] goalTiles = ((NPuzzleSearchProblem) problem).getGoalTiles();
            int[][] tiles = state.getTiles();
            int size = ((NPuzzleSearchProblem) problem).getSize();

            int nMisplacedTiles = 0;
            for (int row = 0; row < size; row++) {
                for (int col = 0; col < size; col++) {
                    if (tiles[row][col] != goalTiles[row][col] && tiles[row][col] != 0) {
                        nMisplacedTiles += 1;
                    }
                }
            }
            return (double) nMisplacedTiles;
        }
